...
module {
  func.func @memory(%arg: !rust.typed.ref<"shared", i32>) -> !rust.typed.rawptr<"const", i32> {
    ...
    %ref = rust.typed.borrow %slot {borrow_kind = "Shared", mutability = "shared"} : !rust.typed.slot<i32> -> !rust.typed.ref<"shared", i32>
    ...
    %field_ref = rust.typed.borrow %field {borrow_kind = "Shared", mutability = "shared"} : !rust.typed.addr<i32> -> !rust.typed.ref<"shared", i32>
    func.call @consume_ref(%field_ref) : (!rust.typed.ref<"shared", i32>) -> ()
    %raw = rust.typed.address_of %slot {mutability = "const", raw_ptr_kind = "Const"} : !rust.typed.slot<i32> -> !rust.typed.rawptr<"const", i32>
