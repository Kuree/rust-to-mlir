...
module {
  rust.typed.func @bad_load {
    ...
    // expected-error@+1 {{result type must match slot element type}}
    %bad = rust.typed.load %slot : <!rust.mir.int<"i32">> -> !rust.mir.int<"i64">
  }
}
...
module {
  rust.typed.func @bad_store {
    ...
    // expected-error@+1 {{value type must match slot element type}}
    rust.typed.store %value, %slot : !rust.mir.int<"i64">, <!rust.mir.int<"i32">>
  }
}
